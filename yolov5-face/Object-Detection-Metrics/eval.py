...
def getBoundingBoxes(gtPath, detPath):
    ...
    folderGT = os.path.join(currentPath, gtPath)
    os.chdir(folderGT)
    files = glob.glob("*.txt")
    files.sort()
    ...
    for f in files:
        ...
        fh1.close()
    # Read detections
    folderDet = os.path.join(currentPath, detPath)
    os.chdir(folderDet)
    files = glob.glob("*.txt")
    files.sort()
...
ap.add_argument("-g", "--gt", required=True,
    help="path to the txt files with ground truth bounding boxes")
ap.add_argument("-d", "--det", required=True,
    help="path to the txt files with detected bounding boxes")
args = vars(ap.parse_args())

# Read txt files containing bounding boxes (ground truth and detections)
boundingboxes = getBoundingBoxes(args["gt"], args["det"])
